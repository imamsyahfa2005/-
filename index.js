...
//
const BotName = 'IS-BOT'; // Nama Bot Whatsapp
...
const kapanbotaktif = 'SẼ GABUT NYA ADMIN'; // Kapan bot lu aktif
const grupch1 = 'https://chat.whatsapp.com/LGEVqaoogWlDQGXMq8YRFb'; // OFFICIAL GRUP LU 1
const grupch2 = 'https://chat.whatsapp.com/Bf6qZU9zXiwBLCMwmwET1A'; // OFFICIAL GRUP LU 2
//
const
{
   WAConnection,
...
axios.get(`https://mhankbarbar.herokuapp.com/nulis?text=${teks}&apiKey=zFuV88pxcIiCWuYlwg57`).then((res) => {
    let hasil = `Hasil Nya Download Sendiri Di Bawah Yah Kak\n\n${res.data.result}`;
...
axios.get(`https://mhankbarbar.herokuapp.com/api/yta?url=${teks}&apiKey=zFuV88pxcIiCWuYlwg57`).then((res) => {
    let hasil = `INI DIA KAK HASIL NYA, TINGGAL DOWNLOAD SENDIRI YAH..\n\nSize: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
axios.get(`https://mhankbarbar.herokuapp.com/api/ytv?url=${teks}&apiKey=zFuV88pxcIiCWuYlwg57`).then((res) => {
    let hasil = `Ini dia kakk hasil nya, download sendiri tah kalo di uploud langsung kami gak kuat:(..\n\nSize: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
conn.sendMessage(id, 'Imam Yah? Imam Itu jomblo tau kalo mau kenalan sama dia chat aja 085779386736..' ,MessageType.text);
}
else if (text == 'imam'){
conn.sendMessage(id, 'Imam Yah? Imam Itu jomblo tau kalo mau kenalan sama dia chat aja 085779386736..' ,MessageType.text);
}
...
if (text.includes("#randomanime"))
   {
    var items = ["anime girl", "anime cantik", "anime", "anime aesthetic", "anime henrai"];
    var cewe = items[Math.floor(Math.random() * items.length)];
    var url = "https://api.fdci.se/rep.php?gambar=" + cewe;
